...
import com.edugroupe.AirlineStatsForm.util.MyXmlStatOutput;
import com.edugroupe.AirlineStatsForm.util.StatsCompagnie;
import com.edugroupe.AirlineStatsForm.util.VolCompagnieClef;

public class SelectJoinCarrierXmlJob  extends Configured implements Tool{
	...
	public static class VolCompagnieReducer extends Reducer<VolCompagnieClef, Text, NullWritable, StatsCompagnie> {
		...
		protected void reduce(VolCompagnieClef clef, Iterable<Text> donnees,
			...
			if (clef.type_clef.get() == VolCompagnieClef.TYPE_VOL) {
				...
				StatsCompagnie sc = new StatsCompagnie(compagnieCourante,
														totalVols,
														total_vols_ok,
														distance_vols / total_vols_ok,
														retard_departs_vols / total_vols_ok,
														retard_arrivees_vols / total_vols_ok);
				context.write(NullWritable.get(), sc);
			}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		ToolRunner.run(new SelectJoinCarrierXmlJob(), args);
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(SelectJoinCarrierXmlJob.class);
		
		job.setInputFormatClass(TextInputFormat.class);
		job.setOutputFormatClass(MyXmlStatOutput.class);
		
		job.setOutputKeyClass(NullWritable.class);
		job.setOutputValueClass(StatsCompagnie.class);
