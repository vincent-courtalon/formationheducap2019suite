...
import com.edugroupe.AirlineStatsForm.util.MyJsonStatOutput;
import com.edugroupe.AirlineStatsForm.util.StatsAeroport;
import com.edugroupe.AirlineStatsForm.util.VolAeroportClef;
import com.edugroupe.AirlineStatsForm.util.VolCompagnieClef;

public class SelectJoinAirportJsonJob  extends Configured implements Tool{
	...
	public static class VolAeroportReducer extends Reducer<VolAeroportClef, Text, NullWritable, StatsAeroport> {
			...
			if (clef.type_clef.get() == VolAeroportClef.TYPE_VOL) {
				if (totalVols > 0) {
					if (total_vols_ok > 0) {
						...
						StatsAeroport sa = new StatsAeroport(clef.code_aeroport.toString(),
															totalVols,
															total_vols_annules * 100.0 / totalVols,
															(int)(retard_departs_vols / total_vols_ok),
															0);
						context.write(NullWritable.get(), sa);
					}
				}
			}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		ToolRunner.run(new SelectJoinAirportJsonJob(), args);
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(SelectJoinAirportJsonJob.class);
		
		job.setInputFormatClass(TextInputFormat.class);
		// format de sortie personnalisé, on écrit du json via notre classe MyJsonStatOutput
		job.setOutputFormatClass(MyJsonStatOutput.class);
		
		job.setOutputKeyClass(NullWritable.class);
		// la sortie n'est plus du texte
		job.setOutputValueClass(StatsAeroport.class);
